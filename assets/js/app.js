...
function showHomeScreen() {
  ...
  // Di DESKTOP: biarkan keduanya tampil
}
  ...
  if (type === 'LIFE_EVENT') {
    ...
    body.innerHTML = `
      <div class="space-y-3">
        ...
        <div class="p-3 bg-slate-50 border border-slate-200 rounded-2xl space-y-1.5">
          <div class="font-extrabold text-brand-navy text-xs flex items-center">
            <i class="fa-solid fa-house-chimney-crack text-amber-600 mr-1.5"></i> BPBD Kota Palopo
          </div>
          <p class="text-[10px] text-slate-500"><i class="fa-solid fa-location-dot mr-1"></i>Jl. Opu Toparambean No. 02, Mungkajang</p>
          <div class="grid grid-cols-2 gap-1.5 pt-1">
            <a href="tel:04713310239" class="bg-brand-navy text-white font-bold py-1.5 rounded-lg text-[10px] flex items-center justify-center space-x-1">
              <i class="fa-solid fa-phone"></i><span>(0471) 3310239</span>
            </a>
            <a href="tel:082194699391" class="bg-amber-600 text-white font-bold py-1.5 rounded-lg text-[10px] flex items-center justify-center space-x-1">
              <i class="fa-solid fa-mobile-screen"></i><span>0821-9469-9391</span>
            </a>
          </div>
          <a href="https://wa.me/6281242362552" target="_blank" class="w-full bg-emerald-600 text-white font-bold py-1.5 rounded-lg text-[11px] flex items-center justify-center space-x-1 mt-1">
            <i class="fa-brands fa-whatsapp"></i><span>Pengaduan WA (0812-4236-2552)</span>
          </a>
        </div>

        <div class="pt-2 border-t border-slate-200">
          <span class="text-[11px] font-extrabold text-slate-500 uppercase tracking-wider block mb-2">Rumah Sakit & Fasilitas Kesehatan</span>
          <div class="space-y-2">
            <div class="p-2.5 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
              <div>
                <strong class="text-xs text-brand-navy block">RSUD Sawerigading</strong>
                <p class="text-[10px] text-slate-500">Jl. Dr. Ratulangi No.1, Wara</p>
              </div>
              <a href="tel:04713312133" class="bg-rose-600 text-white px-2.5 py-1.5 rounded-lg text-[10px] font-bold flex items-center space-x-1 shrink-0">
                <i class="fa-solid fa-phone"></i><span>Call</span>
              </a>
            </div>
            <div class="p-2.5 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
              <div>
                <strong class="text-xs text-brand-navy block">RS Mega Buana</strong>
                <p class="text-[10px] text-slate-500">Jl. Andi Djemma No.99, Wara</p>
              </div>
              <a href="tel:047122266" class="bg-rose-600 text-white px-2.5 py-1.5 rounded-lg text-[10px] font-bold flex items-center space-x-1 shrink-0">
                <i class="fa-solid fa-phone"></i><span>Call</span>
              </a>
            </div>
            <div class="p-2.5 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
              <div>
                <strong class="text-xs text-brand-navy block">RS Bintang Laut</strong>
                <p class="text-[10px] text-slate-500">Jl. Jend. Sudirman No.45, Wara</p>
              </div>
              <a href="tel:047121302" class="bg-rose-600 text-white px-2.5 py-1.5 rounded-lg text-[10px] font-bold flex items-center space-x-1 shrink-0">
                <i class="fa-solid fa-phone"></i><span>Call</span>
              </a>
            </div>
          </div>
...
function renderGisList(filterCategory) {
  ...
  if (!container) return;
  if (typeof gisData === 'undefined' || !Array.isArray(gisData)) {
    container.innerHTML = '<p class="text-xs text-slate-500">Data lokasi belum tersedia.</p>';
    return;
  }
      ...
      <div class="p-2.5 border border-slate-200 rounded-xl bg-slate-50 flex items-center justify-between">
        ...
        <div class="flex space-x-1 shrink-0">
          <a href="${item.mapsUrl}" target="_blank" 
             style="background-color: #e0f2fe; color: #0284c7; border: 1px solid #bae6fd; font-weight: bold; font-size: 0.65rem; padding: 4px 10px; border-radius: 8px; text-decoration: none; display: inline-flex; align-items: center; gap: 4px;">
            <i class="fa-solid fa-diamond-turn-right"></i> Peta
          </a>
          <button onclick="askAI('Info lokasi dan layanan ${item.name}')" 
                  style="background-color: #0f3c5f; color: white; font-weight: bold; font-size: 0.65rem; padding: 4px 10px; border-radius: 8px; border: none; cursor: pointer;">
            Tanya
          </button>
        </div>
      </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Paksa 2 kolom tampil jika di desktop
  if (window.innerWidth >= 1024) {
    const homeScreen = document.getElementById('home-screen');
    const chatScreen = document.getElementById('chat-screen');
    if (homeScreen) homeScreen.classList.remove('hidden');
  ...
  // ==========================================
  // HANDLE RESIZE: paksa 2 kolom di desktop
  // ==========================================
